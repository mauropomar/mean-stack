...
exports.registrarUsuario = async (req, res, next) => {
  try {
    ...
    return next(
      new ErrorResponse(
        "No es posible registrar un usuario " + error.message,
        404
      )
    );
  }
...
exports.login = async (req, res, next) => {
  try {
    ...
    return next(new ErrorResponse("Error en el login " + error.message, 404));
  }
};
...
exports.getUsuario = async (req, res, next) => {
  try {
   ...
   return next(
      new ErrorResponse(
        "Error obteniendo la sesión del usuario " + error.message,
        404
      )
    );
  }
